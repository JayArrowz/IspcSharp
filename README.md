...
## Performance
Benchmarks are run automatically on Linux x64 (AVX2), Windows x64 (AVX2), and macOS ARM64 (Apple Silicon / NEON) using GitHub Actions [here](https://github.com/JayArrowz/IspcSharp/actions/runs/31944847969).
...
- **`[Spmd(Unroll = n)]`** forces the reduction loop's unroll factor (1–4) instead of letting the generator choose. See *Reduction unrolling* below for what it picks and why; the override mainly earns its keep on AVX-512, whose extra registers the generator can't see at compile time. Auto by default.
...
| Area | Types / members |
|---|---|
| Varying types | `VFloat`, `VInt`, `VDouble`, `VLong`, arithmetic/comparison operators, bitwise, shifts, `Load/Store(+Masked)`, `Select`/`Blend`, conversions, `AsInt()`/`AsFloat()` bitcasts, hardware FMA `MulAdd`, unsigned widening multiply-high `VInt.MultiplyHighUnsigned` (`vpmuludq`; pair with `*` for the full 32×32→64 product without 64-bit lanes), variable per-lane shifts (`VInt.Shift*Variable`), narrow-memory bridges (`VInt.LoadZeroExtend`/`LoadSignExtend`/`StoreNarrow` for byte/short buffers) |
...
| Masks | `VMask`, `VMaskD`, `VMaskS`, `VMaskB`, `& \| ! ^`, `Any()`, `AllActive()`, `FirstN(n)`, `AndNot`, `CountActive()`, `ToBitmask()` → `ulong` on all four (ISPC's `lanemask()`, one `movmsk`-class instruction — bit *l* set when lane *l* is active, for walking active lanes with `TrailingZeroCount` or computing a compaction offset; `ulong` because a 512-bit `VMaskB` gang is 64 byte lanes) |
...
**Reduction unrolling:** a straight-line reduction loop (`sum += …`, `Math.Min/Max`) is unrolled with **independent accumulators per copy**, combined and horizontally reduced after the loop. A single accumulator is latency-bound (each `+=`/FMA waits on the previous); independent accumulators fill the pipeline, so wide compute-bound reductions (dot product, matmul, sum of squares) run closer to peak.

The factor is chosen automatically and is **not always 4**. A `double`/`long` accumulator is a `VDouble2`/`VLong2` *pair*, which costs two registers and — because the halves add independently — already supplies two dependency chains before any unrolling. So the generator spends a budget of 4 accumulator registers across all copies: one `float` accumulator unrolls 4×, two `float` or one `double` unroll 2×, two `double` accumulators don't unroll at all. Those come from measurement on L2-resident loops, where unrolling two `double` reductions 2× ran **2.2× slower** than not unrolling, and a Monte Carlo kernel at 4× ran ~20% slower.

Note how far under 16 that budget sits: the limit isn't x86's architectural register file, it's total live values including the body's own temporaries. The generator also skips unrolling entirely when the body is big enough to have its own instruction-level parallelism, charged **transitively through `[SpmdFunction]` calls** — a one-line `Lerp` helper doesn't count against you, a Philox draw does. Control flow in the body disables it too.

Override with **`[Spmd(Unroll = n)]`** (1–4). The unroll is baked into the generated source at compile time while the register count is only known when the JIT runs, so the generator can't adapt to AVX-512's 32 registers for you — though since the measured budget is nowhere near 16, extra registers may not be what's limiting. Measure before changing it. Otherwise transparent, no source change, results still match within the FMA tolerance above.
