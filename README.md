...
| `uniform struct` export arg | a `[SpmdStruct]` value parameter on a `[Spmd]` kernel — stays a scalar struct, field reads broadcast per gang; `[SpmdArray]` members index with **any uniform expression** (no literal restriction); assigning it to a struct local splats every gang (uniform→varying conversion) |
...
tests/IspcSharp.Tests/          xUnit suite (342 tests) covering every feature above, in both
