...
| Masks | `VMask`, `VMaskD`, `VMaskS`, `VMaskB`, `& \| ! ^`, `Any()`, `AllActive()`, `FirstN(n)`, `AndNot`, `CountActive()`, `ToBitmask()` (ISPC's `lanemask()`, one `movmsk`-class instruction — bit *l* set when lane *l* is active, for walking active lanes with `TrailingZeroCount` or computing a compaction offset) |
