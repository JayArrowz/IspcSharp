...
public class ReductionsAndMemoryTests
{
    ...
    public void Reduce_Min_VFloat_FindsMinimum()
    {
        ...
        for (int i = 0; i < w; i++)
            data[i] = (float)rng.NextDouble() * 100;
    ...
    public void Reduce_Max_VFloat_FindsMaximum()
    {
        ...
        for (int i = 0; i < w; i++)
            expected = Math.Max(expected, data[i]);
        Assert.Equal(expected, result, 3);
    }
    ...
    public void Reduce_Min_Masked_IgnoresInactiveLanes()
    {
        ...
        VMask mask = VMask.FirstN(w) & !VMask.FirstN(2);
    ...
    {
        int w = VFloat.LaneCount;
        float[] source = new float[w * 2];
        for (int i = 0; i < source.Length; i++)
            source[i] = i * 10;

        // Indices: 0, 2, 4, 6, ... (every other element)
        int[] idxData = new int[w];
        for (int i = 0; i < w; i++)
            idxData[i] = i * 2;
        VInt indices = VInt.Load(idxData, 0);

        VFloat result = Memory.Gather(source, indices);
    ...
    public void Gather_Float_Masked_UsesFallbackForInactive()
    {
        ...
        VFloat result = Memory.Gather(source, indices, mask, fallback: -1f);
    ...
    {
        int w = VFloat.LaneCount;
        float[] dest = new float[w * 2]; // zero-initialized

        int[] idxData = new int[w];
        for (int i = 0; i < w; i++)
            idxData[i] = i * 2; // write to even positions
        VInt indices = VInt.Load(idxData, 0);

        float[] valData = new float[w];
        for (int i = 0; i < w; i++)
            valData[i] = (i + 1) * 100;
        VFloat values = VFloat.Load(valData, 0);
    ...
    public void Scatter_Float_Masked_OnlyWritesActiveLanes()
    {
        ...
        VInt indices = VInt.ProgramIndex;
        VFloat values = new VFloat(42f);
        VMask mask = VMask.FirstN(3);
    ...
    public void Gather_Int_PicksIndexedElements()
    {
        ...
        VInt result = Memory.Gather(source, indices, VMask.All);
    ...
    public void Gather_Int_Masked_UsesFallbackForInactive()
    {
        ...
        VInt result = Memory.Gather(source, indices, mask, fallback: -1);
    ...
    public void GatherThenScatter_RoundTrips()
    {
        ...
        VInt indices = VInt.ProgramIndex;
        VFloat gathered = Memory.Gather(source, indices);
    ...
    public void Reduce_Min_VInt_Masked_IgnoresInactiveLanes()
    {
        ...
        int[] data = new int[w];
        for (int i = 0; i < w; i++)
            data[i] = i - 5; // lane 0 = -5 (global min)

        // Lane 0 inactive: minimum of remaining lanes is -4.
        VMask mask = VMask.FirstN(w) & !VMask.FirstN(1);
    ...
    public void Transposed_AllocatesAndMatches()
    {
        ...
        float[,] dstF = new float[rows, cols];   // ensure the double overload exists too
        double[,] srcD = new double[rows, cols];
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
                srcD[i, j] = i - j;
        ...
        double[,] dstD = new double[cols, rows];
        Memory.Transpose(srcD, dstD);

        int[,] dstI = new int[cols, rows];
        ...
        float[,] ft = Memory.Transposed(new float[3, 5]);
    ...
    {
        float[] xy = [1, 2, 3, 4, 5, 6];
        SoaFloat2 soa = SoaFloat2.FromInterleaved(xy);

        Assert.Equal(3, soa.Length);
        Assert.Equal([1, 3, 5], soa.X);
        Assert.Equal([2, 4, 6], soa.Y);
    ...
    {
        float[] xyz = [1, 2, 3, 4, 5, 6, 7, 8, 9];
        SoaFloat3 soa = SoaFloat3.FromInterleaved(xyz);

        Assert.Equal(3, soa.Length);
        Assert.Equal([1, 4, 7], soa.X);
        Assert.Equal([2, 5, 8], soa.Y);
        Assert.Equal([3, 6, 9], soa.Z);
