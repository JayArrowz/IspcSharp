...
public static partial class ScaffoldKernels
{
    ...
    public static void AddRowBias(float[,] input, float[] bias, float[,] output, int rows, int cols)
    {
        ...
        {
            foreach (int c in Spmd.Range(cols))
            {
    ...
    public static void ScaleBlocks(float[] data, float[] scales, int blocks, int blockSize)
    {
        for (int bi = 0; bi < blocks; bi++)
        {
            ...
            foreach (int t in Spmd.Range(blockSize))
            {
...
public class ScaffoldAndMatrixTests
{
    ...
    public void MatMulF_Simd_MatchesScalar()
    {
        ...
        float[,] a = RandMatrixF(n, m, 1);
        float[,] b = RandMatrixF(m, p, 2);
        float[,] expected = new float[n, p];
        float[,] actual = new float[n, p];
    ...
    {
        int n = 31, m = 53, p = 17;
        int[,] a = RandMatrixI(n, m, 3);
        int[,] b = RandMatrixI(m, p, 4);
        int[,] expected = new int[n, p];
        int[,] actual = new int[n, p];
    ...
    public void MatMulT_Simd_MatchesScalar_AndPlainMatMul()
    {
        ...
        float[,] bT = Memory.Transposed(b);   // [p, m]
        float[,] actual = new float[n, p];
    ...
    {
        int n = 23, m = 61, p = 19;
        double[,] a = RandMatrixD(n, m, 5);
        double[,] b = RandMatrixD(m, p, 6);
        double[,] expected = new double[n, p];
        double[,] actual = new double[n, p];
    ...
    public void AddRowBias_Simd_MatchesScalar()
    {
        ...
        float[,] input = RandMatrixF(rows, cols, 7);
        float[] bias = new float[rows];
        Random rng = new Random(8);
        for (int r = 0; r < rows; r++)
            bias[r] = (float)(rng.NextDouble() * 10);
    ...
    public void ScaleBlocks_Simd_MatchesScalar()
    {
        ...
        for (int b = 0; b < blocks; b++)
            scales[b] = (float)(rng.NextDouble() * 3);
    ...
    public void Fft_Simd_MatchesScalarReference()
    {
        ...
        float[] real0 = new float[n];
        float[] imag0 = new float[n];
        for (int i = 0; i < n; i++)
        { real0[i] = (float)((rng.NextDouble() * 2) - 1); imag0[i] = 0f; }

        float[] realRef = (float[])real0.Clone();
        float[] imagRef = (float[])imag0.Clone();
        FftReference(realRef, imagRef, n);

        float[] realSimd = (float[])real0.Clone();
        float[] imagSimd = (float[])imag0.Clone();
