...
public class SpmdEngineTests
{
    ...
    public void Foreach_CoversAllElements()
    {
        ...
        Spmd.Foreach(n, (in ctx) =>
        {
            VFloat x = VFloat.LoadMasked(data, ctx.Base, ctx.Active);
    ...
    public void Foreach_IsFullGangFlagCorrect()
    {
        int w = Spmd.LaneCount;
        int n = (w * 2) + 1;
        bool[] fullGangFlags = new bool[(n / w) + 1];
        int flagIdx = 0;

        Spmd.Foreach(n, (in ctx) => fullGangFlags[flagIdx++] = ctx.IsFullGang);
    ...
    public void Foreach_Index_IsCorrectPerLane()
    {
        ...
        Spmd.Foreach(n, (in ctx) => ctx.Index.Store(result, ctx.Base));
    ...
    public void ParallelForeach_ProducesSameResultsAsForeach()
    {
        ...
        float[] resultParallel = new float[n];
        Random rng = new Random(123);
        for (int i = 0; i < n; i++)
            data[i] = (float)rng.NextDouble() * 10f;
    ...
    public void ParallelForeach_SmallCountRunsSingleThreaded()
    {
        ...
        Spmd.ParallelForeach(n, (in ctx) => VFloat.One.StoreMasked(result, ctx.Base, ctx.Active));
    ...
    public void While_NewtonSqrt_ConvergesPerLane()
    {
        ...
        for (int i = 0; i < w; i++)
            input[i] = ((float)rng.NextDouble() * 4) + 0.01f;

        Spmd.Foreach(w, (in ctx) =>
        {
            VFloat x = VFloat.Load(input, ctx.Base);
            VFloat guess = x;
            VFloat err = new VFloat(1f);
            VFloat next = VFloat.Zero;
    ...
    public void While_Break_ExitsLanesEarly()
    {
        ...
        Spmd.Foreach(w, (in ctx) =>
        {
            VInt count = VInt.Zero;
            VInt breakAt = VInt.ProgramIndex + 1; // lane l breaks after l+1 iterations
        ...
        Spmd.Foreach(w, (in ctx) =>
        {
            ...
            _ = Spmd.While(
                ...
                (ref s) =>
                {
                    ...
                    VInt half = VInt.FromFloatTruncate(count.ToFloat() / new VFloat(2f));
                    VInt parity = count - (half * 2);
                    VMask skip = s.Active & VInt.Eq(parity, VInt.Zero);
                    s.Continue(skip);
    ...
    {
        int w = Spmd.LaneCount;
        bool[] returned = new bool[w];

        Spmd.Foreach(w, (in ctx) =>
        {
            VInt count = VInt.Zero;

            LoopState state = Spmd.While(
                ctx.Active,
                active => active,
                (ref s) =>
                {
    ...
    {
        _ = Assert.Throws<InvalidOperationException>(() => _ = Spmd.While(
                VMask.All,
    ...
    {
        int w = Spmd.LaneCount;
        float[] data = MakeRange(w);
        float[] result = new float[w];

        Spmd.Foreach(w, (in ctx) =>
        {
            VFloat x = VFloat.Load(data, ctx.Base);
            int baseIdx = ctx.Base;
            Spmd.If(ctx.Active, x > new VFloat(w / 2f), then: m => (x * 2f).StoreMasked(result, baseIdx, m), @else: m => x.StoreMasked(result, baseIdx, m));
        });
