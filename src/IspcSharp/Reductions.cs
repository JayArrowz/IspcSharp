...
namespace IspcSharp
{
    ...
    {
        /// <summary>Sum of all lanes, ISPC's reduce_add.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static float Add(VFloat v) => Vector.Dot(v.V, Vector<float>.One);

        /// <summary>Sum of active lanes only (inactive lanes contribute 0).</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        {
            int sum = 0;
            for (int l = 0; l < VInt.LaneCount; l++) sum += v.V[l];
            return sum;
        }

        /// <summary>Minimum across all lanes.</summary>
        public static float Min(VFloat v)
        {
            float m = v.V[0];
            for (int l = 1; l < VFloat.LaneCount; l++) m = Math.Min(m, v.V[l]);
            return m;
        }

        /// <summary>Minimum across active lanes (inactive lanes ignored via +Inf).</summary>
        public static float Min(VFloat v, VMask mask)
            => Min(VFloat.Select(mask, v, new VFloat(float.PositiveInfinity)));

        /// <summary>Maximum across all lanes.</summary>
        public static float Max(VFloat v)
        {
            float m = v.V[0];
            for (int l = 1; l < VFloat.LaneCount; l++) m = Math.Max(m, v.V[l]);
            return m;
        }

        /// <summary>Maximum across active lanes (inactive lanes ignored via -Inf).</summary>
        ...
        {
            int m = v.V[0];
            for (int l = 1; l < VInt.LaneCount; l++) m = Math.Min(m, v.V[l]);
            return m;
        }

        /// <summary>Minimum across active lanes (inactive lanes ignored via int.MaxValue).</summary>
        ...
        {
            int m = v.V[0];
            for (int l = 1; l < VInt.LaneCount; l++) m = Math.Max(m, v.V[l]);
            return m;
        }

        /// <summary>Maximum across active lanes (inactive lanes ignored via int.MinValue).</summary>
        public static int Max(VInt v, VMask mask)
            => Max(VInt.Select(mask, v, new VInt(int.MinValue)));

        /// <summary>Sum of all lanes, ISPC's reduce_add (double).</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        {
            double m = v.V[0];
            for (int l = 1; l < VDouble.LaneCount; l++) m = Math.Min(m, v.V[l]);
            return m;
        }
        ...
        {
            double m = v.V[0];
            for (int l = 1; l < VDouble.LaneCount; l++) m = Math.Max(m, v.V[l]);
            return m;
        }
        ...
        /// <summary>Sum of all lanes of a full-float-gang-width double pair.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static double Add(VDouble2 v) => Add(v.Lower) + Add(v.Upper);

        /// <summary>Sum of active lanes only (full-width VMask).</summary>
        ...
        /// <summary>Minimum across all lanes of a double pair.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static double Min(VDouble2 v) => Math.Min(Min(v.Lower), Min(v.Upper));

        /// <summary>Maximum across all lanes of a double pair.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        {
            long sum = 0;
            for (int l = 0; l < VLong.LaneCount; l++) sum += v.V[l];
            return sum;
        }
        ...
        {
            long m = v.V[0];
            for (int l = 1; l < VLong.LaneCount; l++) m = Math.Min(m, v.V[l]);
            return m;
        }

        /// <summary>Minimum across active lanes (inactive lanes ignored via long.MaxValue).</summary>
        ...
        {
            long m = v.V[0];
            for (int l = 1; l < VLong.LaneCount; l++) m = Math.Max(m, v.V[l]);
            return m;
        }

        /// <summary>Maximum across active lanes (inactive lanes ignored via long.MinValue).</summary>
        public static long Max(VLong v, VMaskD mask) => Max(VLong.Select(mask, v, new VLong(long.MinValue)));

        /// <summary>Sum of all lanes of a full-int-gang-width long pair.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>Minimum across all lanes of a long pair.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static long Min(VLong2 v) => Math.Min(Min(v.Lower), Min(v.Upper));

        /// <summary>Maximum across all lanes of a long pair.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    {
        /// <summary>Ordering fence after non-temporal (streaming) stores, so their writes are globally
        /// visible before the buffer is read. Emitted automatically at the end of a <see cref="SpmdAttribute.Streaming"/>
        /// kernel; call it manually if you use <c>StoreNonTemporal</c> directly.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        /// <summary>Gather: result[lane] = source[indices[lane]] for active lanes; <paramref name="fallback"/> otherwise.
        /// Uses AVX-512 (16-wide) or AVX2 (8-wide) hardware gather on net8.0+ when available (indices of
        /// active lanes must still be in bounds).</summary>
        public static unsafe VFloat Gather(ReadOnlySpan<float> source, VInt indices, VMask mask, float fallback = 0f)
        {
            ...
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VFloat.LaneCount == 16)
            {
                ...
                {
                    var idx = System.Runtime.Intrinsics.Vector512.AsVector512(indices.V);
                    var idxLo = idx.GetLower();
                    var idxHi = idx.GetUpper();
                    var mm = System.Runtime.Intrinsics.Vector512.AsVector512(mask.Bits).AsSingle();
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
                    ...
                    return new VFloat(System.Runtime.Intrinsics.Vector512.AsVector(
                        System.Runtime.Intrinsics.Vector512.Create(lo, hi)));
                }
            }
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VFloat.LaneCount == 8)
            {
                ...
                fixed (float* p = source)
                {
                    var idx = System.Runtime.Intrinsics.Vector256.AsVector256(indices.V);
                    var m = System.Runtime.Intrinsics.Vector256.AsVector256(mask.Bits).AsSingle();
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
            ...
            {
                var r = System.Runtime.Intrinsics.Vector128.Create(
            ...
            if (mask.AllActive())
                for (int l = 0; l < w; l++) tmp[l] = source[idxs[l]];
            else
                for (int l = 0; l < w; l++) tmp[l] = mask.IsLaneActive(l) ? source[idxs[l]] : fallback;
        ...
        /// <summary>Scatter: destination[indices[lane]] = values[lane] for active lanes.
        ...
        /// not expose x86 vscatterdps, so this is the fastest scatter available from managed code.</summary>
        public static void Scatter(Span<float> destination, VInt indices, VFloat values, VMask mask)
        {
            ...
            if (TryGetActiveBits(mask, out uint bits))
            {
                while (bits != 0)
                {
                    int l = System.Numerics.BitOperations.TrailingZeroCount(bits);
                    bits &= bits - 1;
        ...
        /// <summary>Gather for int lanes; same hardware paths as the float overload
        /// (AVX2 <c>vpgatherdd</c>, doubled for a 16-wide AVX-512 gang).</summary>
        public static unsafe VInt Gather(ReadOnlySpan<int> source, VInt indices, VMask mask, int fallback = 0)
        {
            ...
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VInt.LaneCount == 16)
            {
                ...
                {
                    var idx = System.Runtime.Intrinsics.Vector512.AsVector512(indices.V);
                    var m = System.Runtime.Intrinsics.Vector512.AsVector512(mask.Bits);
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
            ...
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VInt.LaneCount == 8)
            {
                ...
                {
                    var idx = System.Runtime.Intrinsics.Vector256.AsVector256(indices.V);
                    var m = System.Runtime.Intrinsics.Vector256.AsVector256(mask.Bits);
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
                    var r = System.Runtime.Intrinsics.X86.Avx2.GatherMaskVector256(src, p, idx, m, 4);
                    return new VInt(System.Runtime.Intrinsics.Vector256.AsVector(r));
                }
            }
#endif
        ...
        /// <summary>Scatter for int lanes; same semantics and hardware mask path as the float overload.</summary>
        ...
        /// <summary>Gather for double lanes: result[lane] = source[indices[lane]] for active lanes.
        /// Uses AVX2 vgatherqpd on net8.0+ when available (inactive lanes are never read).</summary>
        public static unsafe VDouble Gather(ReadOnlySpan<double> source, VLong indices, VMaskD mask, double fallback = 0d)
        {
            ...
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VDouble.LaneCount == 8)
            {
                ...
                {
                    var idx = System.Runtime.Intrinsics.Vector512.AsVector512(indices.V);
                    var m = System.Runtime.Intrinsics.Vector512.AsVector512(mask.Bits).AsDouble();
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
                    ...
                    return new VDouble(System.Runtime.Intrinsics.Vector512.AsVector(
                        System.Runtime.Intrinsics.Vector512.Create(lo, hi)));
                }
            }
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VDouble.LaneCount == 4)
            {
                ...
                {
                    var idx = System.Runtime.Intrinsics.Vector256.AsVector256(indices.V);
                    var m = System.Runtime.Intrinsics.Vector256.AsVector256(mask.Bits).AsDouble();
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
            ...
            if (mask.AllActive())
                for (int l = 0; l < w; l++) tmp[l] = source[(int)idxs[l]];
            else
                for (int l = 0; l < w; l++) tmp[l] = mask.IsLaneActive(l) ? source[(int)idxs[l]] : fallback;
        ...
        /// <summary>Scatter for double lanes; same semantics and hardware mask path as the float overload.</summary>
        ...
        /// <summary>Gather for long lanes: result[lane] = source[indices[lane]] for active lanes.
        /// Uses AVX2 vpgatherqq on net8.0+ when available.</summary>
        public static unsafe VLong Gather(ReadOnlySpan<long> source, VLong indices, VMaskD mask, long fallback = 0L)
        {
            ...
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VLong.LaneCount == 8)
            {
                fixed (long* p = source)
                {
                    ...
                    var lo = System.Runtime.Intrinsics.X86.Avx2.GatherMaskVector256(src, p, idx.GetLower(), m.GetLower(), 8);
                    var hi = System.Runtime.Intrinsics.X86.Avx2.GatherMaskVector256(src, p, idx.GetUpper(), m.GetUpper(), 8);
                    return new VLong(System.Runtime.Intrinsics.Vector512.AsVector(
                        System.Runtime.Intrinsics.Vector512.Create(lo, hi)));
                }
            }
            if (System.Runtime.Intrinsics.X86.Avx2.IsSupported && VLong.LaneCount == 4)
            {
                fixed (long* p = source)
                {
                    ...
                    var src = System.Runtime.Intrinsics.Vector256.Create(fallback);
                    var r = System.Runtime.Intrinsics.X86.Avx2.GatherMaskVector256(src, p, idx, m, 8);
                    return new VLong(System.Runtime.Intrinsics.Vector256.AsVector(r));
                }
            }
#endif
        ...
        /// <summary>Scatter for long lanes; same semantics and hardware mask path as the double overload.</summary>
        ...
        private static bool TryGetActiveBits(VMask mask, out uint bits)
        {
            ...
            {
                bits = (uint)System.Runtime.Intrinsics.Vector512.ExtractMostSignificantBits(
                    System.Runtime.Intrinsics.Vector512.AsVector512(mask.Bits));
                return true;
            }
            if (VInt.LaneCount == 8 && System.Runtime.Intrinsics.X86.Avx.IsSupported)
            {
                bits = System.Runtime.Intrinsics.Vector256.ExtractMostSignificantBits(
                    System.Runtime.Intrinsics.Vector256.AsVector256(mask.Bits));
                return true;
            }
            if (VInt.LaneCount == 4 &&
                (System.Runtime.Intrinsics.X86.Sse2.IsSupported || System.Runtime.Intrinsics.Arm.AdvSimd.IsSupported))
            {
                bits = System.Runtime.Intrinsics.Vector128.ExtractMostSignificantBits(
                    System.Runtime.Intrinsics.Vector128.AsVector128(mask.Bits));
                return true;
            }
            bits = 0;
            return false;
        ...
        /// <summary>64-bit-lane variant of <see cref="TryGetActiveBits"/> for VMaskD masks.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool TryGetActiveBitsD(VMaskD mask, out uint bits)
        {
            ...
            {
                bits = (uint)System.Runtime.Intrinsics.Vector256.ExtractMostSignificantBits(
                    System.Runtime.Intrinsics.Vector256.AsVector256(mask.Bits));
                return true;
            }
            if (VDouble.LaneCount == 2 &&
                (System.Runtime.Intrinsics.X86.Sse2.IsSupported || System.Runtime.Intrinsics.Arm.AdvSimd.IsSupported))
            {
                bits = (uint)System.Runtime.Intrinsics.Vector128.ExtractMostSignificantBits(
                    System.Runtime.Intrinsics.Vector128.AsVector128(mask.Bits));
                return true;
            }
            bits = 0;
        ...
        /// <summary>Allocate and return the transpose of a 2-D array.</summary>
        public static float[,] Transposed(float[,] src)
        {
            var dst = new float[src.GetLength(1), src.GetLength(0)];
            Transpose(src, dst);
            return dst;
        }
    }
