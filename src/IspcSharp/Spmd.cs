...
/// </summary>
public readonly ref struct SpmdContext(int @base, VMask active, bool isFullGang)
{
    /// <summary>
    /// Index of the first lane in this "gang" (the base element index).
    /// </summary>
    public readonly int Base = @base;

    /// <summary>
    /// Per-lane element indices: Base + {0,1,...,LaneCount-1}. ISPC's loop variable.
    /// </summary>
    public readonly VInt Index = VInt.ProgramIndex + @base;
    ...
    /// </summary>
    public readonly VMask Active = active;

    /// <summary>
    /// True when every lane maps to a real element (no tail masking needed).
    /// </summary>
    public readonly bool IsFullGang = isFullGang;
}

/// <summary>
/// Kernel body: executes one gang (LaneCount elements) of the iteration space.
/// </summary>
public delegate void SpmdKernel(in SpmdContext ctx);

/// <summary>
/// 2D kernel body: one gang along x, with the (uniform) row index y.
/// </summary>
public delegate void Spmd2DKernel(in SpmdContext ctx, int y);

/// <summary>
/// Condition for a divergent loop: returns the lanes that want another iteration.
/// </summary>
public delegate VMask SpmdCondition(VMask active);

/// <summary>
/// Body of a divergent loop. May shrink the mask (break/return) via <see cref="LoopState"/>.
/// </summary>
...
{
    /// <summary>
    /// Lanes still executing this iteration (after masking out continue for the rest of the body).
    /// </summary>
    public VMask Active { get; set; }

    /// <summary>
    /// Lanes that have exited the loop entirely (break or return).
    /// </summary>
    public VMask Exited { get; set; }

    /// <summary>
    /// Lanes that returned from the whole kernel (propagates outward).
    /// </summary>
    public VMask Returned { get; set; }

    internal VMask _continued;

    /// <summary>
    /// SPMD <c>break</c>: deactivate <paramref name="lanes"/> for all remaining iterations.
    /// </summary>
    ...
    /// <summary>
    /// SPMD <c>continue</c>: deactivate <paramref name="lanes"/> for the rest of THIS iteration only.
    /// </summary>
    ...
    /// <summary>
    /// SPMD <c>return</c>: deactivate <paramref name="lanes"/> permanently, and record them as returned.
    /// </summary>
...
{
    /// <summary>
    /// SIMD lane count on this machine (floats per instruction).
    /// </summary>
    ...
    /// <summary>
    /// Half-open range [start, end), generator marker, also runs scalar if called directly.
    /// </summary>
    ...
    /// <summary>
    /// Multi-core 2D tiled foreach: tiles are distributed across the thread pool.
    /// </summary>
    public static void ParallelForeach2D(int width, int height, Spmd2DKernel kernel, int tileW = 64, int tileH = 64)
    {
        ...
        _ = Parallel.For(0, tilesX * tilesY, t =>
        {
